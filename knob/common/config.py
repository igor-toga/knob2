...
service_opts = [
    ...
    cfg.StrOpt('knob_metadata_server_url',
               help=_('URL of the Knob metadata server. '
                      'NOTE: Setting this is only needed if you require '
                      'instances to use a different endpoint than in the '
                      'keystone catalog')),
    cfg.StrOpt('knob_waitcondition_server_url',
               help=_('URL of the Knob waitcondition server.')),
    cfg.StrOpt('knob_watch_server_url',
               default="",
               help=_('URL of the Knob CloudWatch server.')),
    ...
    cfg.StrOpt('knob_stack_user_role',
               default="knob_stack_user",
               help=_('Keystone role for knob template-defined users.')),
    cfg.StrOpt('stack_user_domain_id',
               deprecated_opts=[cfg.DeprecatedOpt('stack_user_domain',
                                                  group=None)],
               help=_('Keystone domain ID which contains knob '
    ...
    cfg.StrOpt('stack_user_domain_name',
               help=_('Keystone domain name which contains knob '
...
engine_opts = [
    ...
    cfg.ListOpt('trusts_delegated_roles',
                default=[],
                help=_('Subset of trustor roles to be delegated to knob.'
                       ' If left unset, all roles of a user will be'
                       ' delegated to knob when creating a stack.')),
    ...
    cfg.BoolOpt('enable_cloud_watch_lite',
                default=False,
                help=_('Enable the legacy OS::Knob::CWLiteAlarm resource.')),
    ...
    cfg.BoolOpt('observe_on_update',
                default=False,
                help=_('On update, enables knob to collect existing resource '
    ...
    cfg.StrOpt('default_software_config_transport',
               ...
               help=_('Template default for how the server should receive the '
                      ...
                      'keypair (requires enabled knob-api-cfn). '
                      'POLL_SERVER_HEAT will allow calls to the '
                      'Knob API resource-show using the provided keystone '
    ...
    cfg.StrOpt('default_deployment_signal_transport',
               ...
               help=_('Template default for how the server should signal to '
                      'knob with the deployment output values. CFN_SIGNAL '
                      'will allow an HTTP POST to a CFN keypair signed URL '
                      '(requires enabled knob-api-cfn). '
                      ...
                      'HEAT_SIGNAL will allow calls to the Knob API '
    ...
    cfg.StrOpt('default_user_data_format',
               ...
               help=_('Template default for how the user_data should be '
                      ...
                      'user_data is bundled as part of the knob-cfntools '
    ...
    cfg.BoolOpt('stack_scheduler_hints',
                ...
                help=_('When this feature is enabled, scheduler hints'
                       ' identifying the knob stack context of a server'
                       ' or volume resource are passed to the configured'
                       ' schedulers in nova and cinder, for creates done'
                       ' using knob resource types OS::Cinder::Volume,'
                       ' OS::Nova::Server, and AWS::EC2::Instance.'
                       ' knob_root_stack_id will be set to the id of the'
                       ' root stack of the resource, knob_stack_id will be'
                       ' set to the id of the resource\'s parent stack,'
                       ' knob_stack_name will be set to the name of the'
                       ' resource\'s parent stack, knob_path_in_stack will'
                       ' be set to a list of comma delimited strings of'
                       ' stackresourcename and stackname with list[0] being'
                       ' \'rootstackname\', knob_resource_name will be set to'
                       ' the resource\'s name, and knob_resource_uuid will be'
...
knob_client_opts = [
    cfg.StrOpt('url',
               default='',
               help=_('Optional knob url in format like'
...
revision_opts = [
    cfg.StrOpt('knob_revision',
               default='unknown',
               help=_('Knob build revision. '
...
def startup_sanity_check():
    if (not cfg.CONF.stack_user_domain_id and
        ...
        # FIXME(shardy): Legacy fallback for folks using old knob.conf
        # files which lack domain configuration
        LOG.warning(_LW('stack_user_domain_id or stack_user_domain_name not '
                        'set in knob.conf falling back to using default'))
    else:
        ...
        if not (domain_admin_user and domain_admin_password):
            raise exception.Error(_('knob.conf misconfigured, cannot '
    ...
    if auth_key_len in (16, 24):
        ...
        raise exception.Error(_('knob.conf misconfigured, auth_encryption_key '
...
def get_client_option(client, option):
    ...
    try:
        ...
        cfg.CONF.import_opt(option, 'knob.common.config',
                            group=group_name)
    ...
    cfg.CONF.import_opt(option, 'knob.common.config', group='clients')
